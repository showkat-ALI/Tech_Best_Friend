...
import {useGetCategoriesQuery,useGetCourseByCategoryQuery,useGetAllActiveCourseQuery} from "../../../../feature/api/dashboardApi"
import FirstTab from "../bootCampsTabsOption/cardsForFirstTab/FirstTab";
import SecondTab from "../bootCampsTabsOption/cardsForSecondTab/SecondTab";
import FourthTab from "../bootCampsTabsOption/cardForFourthTab/FourthTab";
import AllCources from "../bootCampsTabsOption/allCources/AllCources";
...
const BootCampsTab = () => {
  ...
  const { data:activeCourse, isSuccess:activeCourseSuccess, isError:activeCourseError, isLoading:activeCourseLoading } = useGetAllActiveCourseQuery({});
  const { data:course, isSuccess:courseIsSuccess, isError:courseError, isLoading:courseLoading } = useGetCourseByCategoryQuery(toggletabs);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 xl:gap-7 p-4">
          {
           courseLoading ? 
                    <div>Loading...</div> : 
            courseIsSuccess &&  course.data.courses.length > 0 ? course.data.courses.map((item:any) => (
              <BootCampsCard
                   key={item._id}
                   id={item._id}
                   name={item.title}
