...
const Creation3 = (props:StepPropss) => {
  ...
  return (
    <>
     ...
     {
      EditshowModal && <EditModuleModal id={ModuleId} EditshowModal={EditshowModal} setEditShowModal={setEditShowModal}  />
     }  
     <form onSubmit={handleSubmit(submitThirdStep)}>
      <div className='Curriculum p-3 mt-5'>
          ...
          <div className=''>
             ...
             { isSuccess && data.data.modules.map(({id,name,assignments,quizzes,videos,slides,duration}:{duration:number;id:string,name:string,index:string,assignments:string[];quizzes:string[],videos:string[],slides:string[]},index:string) => (
                  <Module duration={duration} setModuleId={setModuleId} setEditShowModal={setEditShowModal} id={id} name={name} index={index} assignments={assignments} quizzes={quizzes} videos={videos} slides={slides}/>
             ))
             }  
