...
import { useGetAssignmentQuery } from "../../../../../../../feature/api/dashboardApi";
import { useUpdateModuleMutation } from "../../../../../../../feature/api/dashboardApi";
import { toast } from "react-toastify";
import ButtonLoader from "../../../../../../utils/loaders/ButtonLoader";
import Link from 'next/link'

const AssignmentCategory = ({id,setShowModal}:{id:string,setShowModal:any}) => {
  const { data, isSuccess, isError, isLoading } = useGetAssignmentQuery({});
  const [updateModule, { error, data:moduleData, isLoading:loadingModule, isSuccess:moduleisSuccess}] = useUpdateModuleMutation();
  ...
  return (
    <>
          <div>
                <div>
                     <p className='text-[#8A92A6] text-[16px] mb-3 leading-[19px]'>Select the assignment you want to associate with this module or add an assignment by selecting “Create Assignment”.</p>
                </div>
                <div className='bg-[#fff] p-3 px-5 rounded' style={{boxShadow: "0px 1px 15px rgba(0, 0, 0, 0.15)"}}>
                         <Link href={`/dashboard/instructor/assignmentcreation`}><h2 className='cursor-pointer text-[#3A57E8] underline font-medium mb-2 text-base'>Create Assignment</h2></Link>
                         <div>
                             <ul className='flex flex-col gap-[10px] text-[#8A92A6] text-[15px] mb-2 h-[12rem] overflow-y-scroll	'>
                                 {isLoading ? 
                                         ...
                                         :isError ? 
                                              <div>Error....</div>
                                             :isSuccess && data?.data?.assignments && data.data.assignments.length > 0 ?
                                             data.data.assignments.map(({name,id}:{name:string;id:string},index:string) => (<li onClick={() => clickAssignment(id)} className={`${activeClass == id && "text-[#3A57E8]"} hover:text-[#da7b4f] cursor-pointer`}>Assignment {index + 1} - {name}</li>))  
          ...
          <div className='flex flex-col mt-3 p-4'>
                        ...
                        <select className='w-[16rem] h-[3rem] mb-2 border-none' style={{boxShadow: "0px 1px 15px rgba(0, 0, 0, 0.15)"}}>
