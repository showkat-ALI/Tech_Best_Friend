...
import { SuccessCreate,DelEditCourse } from "../../../../../../feature/course/courseSlice";
//icon
...
const Creation4 = (props:StepPropss) => {
    ...
    const {data:faqsData,isError:faqsisError,error:faqsError,isLoading:faqsLoading,isSuccess:faqsIsSuccess} = useGetFaqsQuery(id)
    const { register,control,setValue, handleSubmit, formState: { errors } } = useForm<props>({
      defaultValues: {
        tags:formData.tags,
    ...
    <> 
      ...
      <form onSubmit={handleSubmit(courseRegister)}>
       <div className='p-3 mt-5'>
           <h2 className='font-semibold text-2xl mb-5'>Additional Information</h2>
           <div>
               <div className='heading_upload_lectrue flex justify-between items-center mb-4'>
                  ...
                  <button type="button" className="bg-[#EBEEFD] border border-[#3A57E8] px-3 py-2 rounded flex items-center gap-[6px]"><Image src={plusIconBg} onClick={() =>setmoduleModalShow(!moduleModalShow)} />Add Question</button>
               </div>
  {
      faqsIsSuccess &&   faqsData.data.faqs.map((item:any) => (
               <div className='p-4 mb-4' style={{boxShadow:'0px 1px 15px rgba(0, 0, 0, 0.15)'}}>
               ...
               <div className='upload_image_details flex justify-between bg-[#CDEBEC] mt-10 mb-8' style={{padding: '12px 21px'}}>
                  <div className='flex items-center gap-2'>
                      <span className='xsm:text-[9px] sm:text-[10px] lg:text-base font-normal text-[#056C71]'>Accepted file types:</span>
                      
                  </div>
                  <div className='flex items-center gap-2'>
                      <span className='xsm:text-[9px] sm:text-[10px] lg:text-base text-[#056C71]'>Accepted file types:</span>
                     
                  </div>
                  <div className='flex items-center gap-2'>
                        
                         <span className='xsm:text-[9px] sm:text-[10px] lg:text-base text-black'>PNG,JPG,JPEG</span>
                 </div>
              </div>

               <div className='message flex flex-col mb-3'>
                    <label className='text-base font-medium mb-3'>Message to a reviewer</label>
                    <textarea {...register("msgtoreviewer", { required: true })} placeholder='Write a message'  className='p-3 border-none' style={{boxShadow:"0px 1px 15px rgba(0, 0, 0, 0.15)"}}></textarea>
                    {errors.msgtoreviewer && <InputErrorMessage  message={"Enter message reviewer"} />}
               </div>
             
               <div className='btn flex justify-end gap-5 mt-8'>
                   <button className='xsm:w-full lg:w-[8rem] bg-[#EBEEFD] border border-[#3A57E8] py-2 px-4 rounded' onClick={onPrev}>Previous</button>
                    <button className='flex xsm:w-full lg:w-[9rem] bg-[#3A57E8] py-2 px-4 text-[#fff] rounded' type='submit'>
                        {
