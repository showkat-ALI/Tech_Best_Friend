...
const AssignmentCreation = () => {
  ...
  const { register, handleSubmit,setValue, formState: { errors } } = useForm<assignmentType>({
    defaultValues: {
      name:formData.name,
  ...
  const [createAssignment,{ error, data,isLoading, isSuccess, isError }] = useCreateAssignmentMutation();

  const assignmentHandler = (data:assignmentType) => {
    if(!data.fileUrl){
      alert("please select assignmen file")
    }
    console.log("f")
  ...
  return (
    <div className="grid  lg:grid-cols-12 w-full justify-between gap-x-8">
        ...
        <form  onSubmit={handleSubmit(assignmentHandler)}>
             <AssignementNaming register={register} setValue={setValue} errors={errors}/>
             <AssignmentFile setValue={setValue} errors={errors} register={register}/>
