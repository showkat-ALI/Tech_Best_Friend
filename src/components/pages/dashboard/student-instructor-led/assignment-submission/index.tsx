...
export default function AssignmentSubmission() {
  ...
  const { data:AllSubmitAssignment, isSuccess:AllSubmitAssignmentIsSuccess, isError:AllSubmitAssignmentIsError, isLoading:AllSubmitAssignmentIsLoading } = useGetAllSubmitAssignmentQuery({});
  const [submitAssignment, { error, data, isLoading, isSuccess,isError }] = useSubmitAssignmentMutation();
  ...
  const onQuestionSubmit = (data:any) => {
    ...
    submitAssignment({course:courseId,assignment:assignmentId,text:data.text,comment:data.comment,fileUrl:data.fileUrl})
  };
  ...
  useEffect(() => {
   const test = async () => {
    try{
      const a =   AllSubmitAssignment.data.subAssignments.filter((student:any) => student.assignment === assignmentId && student.student  === studentId && student.course === courseId)
      .map((student:any) => {
       router.push(`/dashboard/instructor/students/assignmentsubmit/${student._id}`)
      })
    }catch(err){
          ...
          <div className="my-5">
            <div>
              ...
              <div>
                ...
                />
                 <div>
                           {errors.text && <InputErrorMessage message="Enter description"/>}
                </div>
              </div>
            </div>
          </div>
              ...
              <div className="xsm:p-3 lg:p-0 lg:pr-[6rem] py-11 bg-white rounded-lg">
                              ...
                              >
                                ...
                                <div className="flex justify-center mt-1 items-center">
                                  ...
                                  >
                                    ...
                                    <input  {...register("fileUrl",{required:true})}  onChange={fileGet} type="file" className="hidden" name="filess"/>
                                  </label>
                                  <span className='ml-2 text-[#727272] text-sm'>{Filename? Filename: "No file choosen"}</span>
                               
                                </div>
                                <div className='pt-5'>
                                     <p className='text-[#3A57E8] font-medium mt-3'>{uploadLoading && "uploading..."}{isUploadSuccess && "Upload Completed"}</p>
                ...
                <div className=" mt-10 lg:pl-[2rem]">
                  ...
                  <div className="my-5 lg:w-[90%] mb-6 pb-7">
                    ...
                    />
                    <div>
                          {errors.comment && <InputErrorMessage  message={"Enter your comment"} />}
                   </div>
                  </div>
                </div>
