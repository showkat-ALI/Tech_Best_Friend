...
function Table(props: IStudentProps) {
  const { profile: { avatar, firstName, lastName, userName, studentType }, id, contact, email, status, country, createdAt, currentJob } = props
    ...
    <tr className="border-b">
      ...
      <td className="py-4 px-6">
        <div className="flex justify-center space-x-6">
          ...
          <button onClick={() => setShowAcceptStudentModal(true)} className="bg-[#3A57E8] rounded text-white px-3 lg:px-5 py-1 lg:py-2 ">
            Accept
          </button>
          <button onClick={() => setShowRejectStudentModal(true)} className="bg-[#C03221] rounded text-white px-3 lg:px-5 py-1 lg:py-2 ">
            Reject
          </button>
...
function AdmissionTable() {
  const { data, isLoading, isError, isSuccess, error } = useGetAllAdmissionRequestQuery({})
          ...
          <table className="w-full text-[16px] md:text-[18px] text-left">
            ...
            <tbody className="text-[#232D42]">
              {data.data.users.map(({
                ...
                createdAt,
                currentJob
              }: { _id: string, firstName: string, lastName: string, avatar: string, email: string, studentType: string, phone: string, country: string, status: string, userName: string, createdAt: Date, currentJob: string }) => (
                <Table
                  key={_id}
