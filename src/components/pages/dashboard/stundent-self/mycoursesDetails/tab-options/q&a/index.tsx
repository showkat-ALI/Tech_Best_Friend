...
export default function index({enrollmentData}:{enrollmentData:any}) {
  const [createQuestionCourse,{ error, data, isLoading, isSuccess, isError }] = useCreateQuestionCourseMutation();
  ...
  const questionSubmit = async (data:props) => {
    await createQuestionCourse({name:data.name,email:data.email,question:data.question,course:enrollmentData.course.id});
          ...
          <div className="col-span-12 lg:col-span-12">
            <div>
              ...
              <div className="flex items-center gap-2 my-4">
                ...
                <span className="text-[#8A92A6]">({enrollmentData.course.QA.length})</span>
              </div>
              <div className="pt-3">
        { 
          enrollmentData.course.QA.map((item:any) => (
                <div className="flex gap-4 mb-7 w-full">
                  <div>
                    <Image src={item.user.avatar} alt="" width={40} height={40} className="rounded-full"/>
                  </div>
            ...
            <form onSubmit={handleSubmit(questionSubmit)}> 
                <div className="flex gap-5 pt-4">
                  <div className="flex-col flex w-[50%]">
                    ...
                    />
                    <div>
                       {errors.name && <InputErrorMessage  message={"Enter Name"} />}
                    </div>
                  </div>
                  <div className="flex flex-col w-[50%]">
                    ...
                    />
                    <div>
                        {errors.email && <InputErrorMessage  message={"Enter Email"} />}
                    </div>
                  </div>
                </div>
                ...
                <button  type="submit" className="flex justify-center bg-[#3A57E8] text-white rounded px-6 py-2">
                  {
